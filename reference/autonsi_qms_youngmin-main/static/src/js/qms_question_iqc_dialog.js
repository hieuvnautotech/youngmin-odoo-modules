...
class QMSQuestionIQCDialog extends Component {
    static template = "qms_question_iqc_dialog_template";
    ...
    setup() {
        ...
        console.log(context.check_date)
        ...
        this.state = useState({
            ...
            'confirm_staff_id': context.confirm_staff_id ?? context.employee_list[0].id,
            'confirm_staff_name': context.confirm_staff_name || "",
            'staff_id': context.staff_id ?? context.employee_list[0].id,
            'staff_name': context.staff_name || "",
            'final_result': context.final_result || "OK",
            // 'check_date':  context.check_date ? new Date(context.check_date + "Z").toISOString().split('T')[0] :  new Date().toISOString().split('T')[0],
            ...
            'stock_move_line_id': context.stock_move_line_id,
            'qty': Math.round(Number(context.qty ?? 1.0) * 100) / 100,
        });

        this.state.columns = this.props.context.columns;
    ...
    async saveData() {
        ...
        await this.orm.call('mes.qc_form.history', 'create_history_iqc', [], { inspectionData: this.state });
    ...
    async createData() {
        try {
            ...
            await this.orm.call('mes.qc_form.history', 'create_history_iqc_data', [], { inspectionData: this.state });
...
function mesQcPreviewAction(env, action) {
    const dialog = env.services.dialog;

    dialog.add(QMSQuestionIQCDialog, {
        context: action.context,
    });
}


actionRegistry.add("qms_question_iqc_action", mesQcPreviewAction);