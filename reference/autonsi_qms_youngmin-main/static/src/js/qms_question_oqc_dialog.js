...
class QMSQuestionOQCDialog extends Component {
    static template = "qms_question_oqc_dialog_template";
    ...
    setup() {
        ...
        this.state = useState({
            ...
            'is_resistance': context.is_resistance || false,
            'is_final_process': context.is_final_process || false,
            'rev_no': context.rev_no || "",
    ...
    async saveData() {
        // try {
            ...
            await this.orm.call('mes.qc_form.history', 'create_history_oqc', [], {inspectionData: this.state});
...
function mesQcPreviewOQCAction(env, action) {
    const dialog = env.services.dialog;

    dialog.add(QMSQuestionOQCDialog, {
        context: action.context,
    });
}


actionRegistry.add("qms_question_oqc_action", mesQcPreviewOQCAction);