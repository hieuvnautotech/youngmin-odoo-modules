...
class QMSQuestionItemQCDialog extends Component {
    static template = "qms_question_item_qc_dialog_template";
    ...
    setup() {
        ...
        this.state = useState({
            ...
            'mrp_material_id': context.mrp_material_id,
            'mrp_mo_material_id': context.mrp_mo_material_id,
    ...
    async saveData() {
        // try {
            ...
            // await this.orm.call('mes.qc_form.history', 'create_history_item_qc', [], {inspectionData: this.state});

            if (this.state.mrp_material_id) {
                await this.orm.call('mes.qc_form.history','create_history_item_qc',[],{ inspectionData: this.state });
            } else {
                await this.orm.call('mes.qc_form.history','create_history_item_qc_mo',[],{ inspectionData: this.state });
            }
...
function mesQcPreviewItemQCAction(env, action) {
    const dialog = env.services.dialog;

    dialog.add(QMSQuestionItemQCDialog , {
        context: action.context,
    });
}


actionRegistry.add("qms_question_item_qc_action", mesQcPreviewItemQCAction);